 ...
 * @author Acer
 */
public class ViewConsignment extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int consignmentPerPage = 5;

        ConsignmentDAO consignmentDAO = new ConsignmentDAO();
//        ProductDAO productDAO = new ProductDAO();

        int totalConsignment = consignmentDAO.getTotalConsignment();

        int endPage = totalConsignment / consignmentPerPage;
        if (totalConsignment % consignmentPerPage != 0) {
            endPage++;
        }

        List<Consignment> list = consignmentDAO.getConsignmentByPage(index, consignmentPerPage);
