<%-- 
    Document   : OrderImportProduct.jsp
    Created on : Oct 7, 2024, 9:55:27 PM
    Author     : Admin
--%>
                ...
                <li class="nav-item">
                    ...
                    <div class="collapse" id="collapseSanPham">
                        <ul class="list-unstyled ps-3">
                            ...
                            <!--<li><a href="#" class="nav-link">Nhà cung cấp</a></li>-->
                        </ul>
                    </div>
                </li>
...
<!--                            <li>
                            ...
                            <li><a class="dropdown-item d-flex align-items-center" href="${pageContext.request.contextPath}/LogoutController">
        ...
        <div class="main-content" id="main-content">
            ...
            <div class="container-fluid">
                <div class="d-flex justify-content-start align-items-center mb-3">
                    <form action="OrderImportProduct" method="post" enctype="multipart/form-data">
                        <input type="hidden" name="service" value="uploadFile">
                        <label for="orderFile">Tải lên file đơn hàng:</label>
                        <input type="file" name="orderFile" accept=".csv, .xls, .xlsx" required>
                        <button type="submit">Tải lên và Tạo đơn hàng</button>
                    </form>

                </div>
                
                <div class="d-flex justify-content-start align-items-center mb-3">
                    <form action="${pageContext.request.contextPath}/OrderImportProductList" method="post">
                        <input type="hidden" name="service" value="exportConsignment"> <!-- Gửi service để xác định hành động -->
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <ul class="nav nav-tabs">
                        ...
                        <!--                        <li class="nav-item"><a class="nav-link" href="#">Chưa Nhập</a></li>
                                                <li class="nav-item"><a class="nav-link" href="#">Hoàn thành</a></li>-->
                    </ul>

                    <a class="nav-link" href="${pageContext.request.contextPath}/OrderImportProduct">
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <!-- Form Tìm Kiếm -->
                    <form action="${pageContext.request.contextPath}/OrderImportProductList" method="get" class="d-flex" style="flex: 1;">
                        <input type="hidden" name="service" value="searchConsignment">
                        <div class="input-group me-2" style="width: 100%;">
                            <span class="input-group-text bg-white border-end-0">
                                <i class="bi bi-search"></i>
                            </span>
                            <input type="text" class="form-control border-start-0" placeholder="Tìm id lô hàng, tên NCC" name="searchQuery" aria-label="Tìm kiếm">
                            <button class="btn btn-primary" type="submit">Tìm kiếm</button>
                        </div>
                    </form>

                    <!-- Form Lọc -->
                    <form action="${pageContext.request.contextPath}/OrderImportProductList" method="get" class="d-flex align-items-center ms-3">
                ...
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Giá trị lô hàng</th>
                        </tr>
                    </thead>
                    <tbody>
                        <c:forEach var="consignment" items="${listC}">
                            <tr>
                                <td>
                                    <a href="${pageContext.request.contextPath}/OrderImportProductList?service=consignmentDetail&id=${consignment.id}" class="nav-link">
                                        <i class="bi bi-eye text-primary"></i>
                                    </a>
                                </td>
                                <td name="id">${consignment.id}</td>
                                <td name="deliveryDate">${consignment.deliveryDate}</td>
                                <td>
                                    <c:choose>
                                        <c:when test="${consignment.status == 0}">
                                            <span class="badge bg-warning">Đang xử lý</span>
                                        </c:when>
                                        <c:when test="${consignment.status == -1}">
                                            <span class="badge bg-success">Hoàn thành</span>
                                        </c:when>
                                        <c:when test="${consignment.status == -2}">
                                ...
                                <td name="supplierName">${consignment.supplierName}</td> 
                                <td name="price">${consignment.importPrice} VNĐ</td>
                            </tr>
                        </c:forEach>
                        ...
                        <ul class="pagination">
                            <c:forEach var="i" begin="1" end="${totalPages}">
                                <li class="page-item ${currentPage == i ? 'active' : ''}">
                                    <a class="page-link" href="${pageContext.request.contextPath}/OrderImportProductList?page=${i}&statusFilter=${statusFilter}">${i}</a>
                                </li>
                            </c:forEach>
                        </ul>
                    </nav>
